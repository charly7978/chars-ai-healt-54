# Script Simple de Configuración del Sistema Automático de Git
Write-Host "=== CONFIGURACIÓN SIMPLE DEL SISTEMA AUTOMÁTICO DE GIT ===" -ForegroundColor Cyan
...
powershell.exe -ExecutionPolicy Bypass -File "scripts/auto-fix-git.ps1" -CommitMessage "Auto-fix pre-commit: $(git log -1 --pretty=format:'%s')"

if [ `$? -eq 0 ]; then
    echo "Hook pre-commit completado exitosamente"
    exit 0
else
    echo "Error en hook pre-commit"
    exit 1
fi
"@
...
$PostCommitContent = @"
#!/bin/sh
# Hook post-commit para notificaciones
